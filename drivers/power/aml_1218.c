...
int aml1218_write16(uint32_t add, uint32_t val)
{
...
int aml1218_writes(int add, uint8_t *buff, int len)
{
    int ret;
    uint8_t buf[MAX_BUF] = {};
    buf[0] = add & 0xff;
...
int aml1218_set_bits(int add, uint8_t bits, uint8_t mask)
{
    uint8_t val;
    aml1218_read(add, &val);
    val &= ~(mask);                                         // clear bits;
    val |=  (bits & mask);                                  // set bits;
    return aml1218_write(add, val);
}
...
int aml1218_get_charge_status(int print)
{
    ...
    aml1218_set_bits(0x003b, (reg_all & 0x02000000) ? 0x00 : 0x40, 0x40);
    aml1218_set_bits(0x0044, (reg_all & 0x02000000) ? 0x00 : 0x40, 0x40);
    aml1218_write(0x011d, (reg_all & 0x02000000) ? 0x02 : 0x00);
...
static int aml1218_get_coulomber(void)
{
    uint8_t buf[8]= {};
    int ret;
    int charge_result;
...
int aml1218_set_gpio(int pin, int val)
{
    ...
    if (val < 2) {
        ...
        printf("%s, not support value for 1218:%d\n", __func__, val);
        return -1;
    }
    ...
    printf("%s, GPIO:%d, val:%d\n", __func__, pin, val);
    if (val) {
        aml1218_write16(0x0084, data);
    } else {
        aml1218_write16(0x0082, data);
    }
    return 0;
#endif
}

int aml1218_get_gpio(int pin, int *val)
{
    int ret;
    uint8_t data;
...
static int aml1218_cal_ocv(void)
{
    ...
    ibat = aml1218_get_battery_current();
    charge_status = aml1218_get_charge_status(0);
    if (charge_status == 1) {
...
int aml1218_get_ocv(int charge_status, int rdc)
{
    int vbat;
    int ibat;
    int ocv;
...
int aml1218_get_charging_percent()
{
    int rest_vol;
#ifdef CONFIG_UBOOT_BATTERY_PARAMETERS
    int i;
    ...
    int charge_status = aml1218_get_charge_status(1);
    int para_flag;
...
{
    unsigned char val;
    if (dcdc < 0 || dcdc > AML1218_DCDC3 || en > 1 || en < 0) {
        return ;    
    }
    switch(dcdc) {
    case AML1218_DCDC1:
        aml1218_read(0x0036, &val);
        if (en) {
            val |=  (1 << 5);                                   // pfm mode
        } else {
            val &= ~(1 << 5);                                   // pwm mode
        }
        aml1218_write(0x0036, val);
        break;

    case AML1218_DCDC2:
        aml1218_read(0x0044, &val);
        if (en) {
            val |=  (1 << 5);    
        } else {
            val &= ~(1 << 5);   
        }
        aml1218_write(0x0044, val);
        break;

    case AML1218_DCDC3:
        aml1218_read(0x004e, &val);
        if (en) {
            val |=  (1 << 7);    
        } else {
            val &= ~(1 << 7);   
        }
        aml1218_write(0x004e, val);
        break;

    case AML1218_BOOST:
        aml1218_read(0x0028, &val);
        if (en) {
            val |=  (1 << 0);    
        } else {
            val &= ~(1 << 0);   
        }
        aml1218_write(0x0028, val);
        break;
...
int aml1218_set_dcdc_voltage(int dcdc, uint32_t voltage)
{
    ...
    if (idx_to_12 >= 0) {
        ...
        {
              aml1218_set_bits(addr, (uint8_t)tmp1, 0x1c); //set bit[4:2]
              aml1218_set_bits(addr, (uint8_t)tmp2, 0xe0);//set bit[7:5] 
        }
        
        aml1218_set_bits(addr1, 0x40, 0x40);//set bit[6]     
...
int aml1218_read16(uint32_t add, uint16_t *val)
{
    int ret;
    uint8_t buf[2] = {};
    buf[0] = add & 0xff;
...
void aml1218_set_long_press(int ms)
{   
    uint16_t val;
    uint16_t tmp;
...
void check_boot_up_source(void)
{
    ...
    aml1218_read(0x8A, &val1);
    aml1218_read(0xE0, &val2);
...
int aml1218_check_fault(void)
{
    ...
    aml1218_read(0x87, &val0);
    aml1218_read(0x88, &val1);
...
int aml1218_set_charge_end_rate(int rate)
{
    ...
    return aml1218_set_bits(0x0129, val, 0x10);
}

int aml1218_set_trickle_time(int min)
{
    int val;
    switch (min) {
    ...
    return aml1218_set_bits(0x012a, val, 0x0c);
}

int aml1218_set_rapid_time(int min)
{
    int val;
    switch (min) {
    ...
    return aml1218_set_bits(0x0129, val, 0x0c);
}
...
int aml1218_init(void)
{
    ...
    aml1218_write(0x009E, 0x18);//enable VBAT measure result average 4 samples
    aml1218_write(0x009F, 0x60);//enable IBAT measure result average 4 samples
    aml1218_write(0x009A, 0x20);
    aml1218_write(0x00B8, 0x00);
    aml1218_write(0x00A9, 0x8f);

    aml1218_write(0x00A0, 0x00);//select auto-sampling timebase is 2ms
    aml1218_write(0x00A1, 0x15);//set the IBAT measure threshold and enable auto IBAT +VBAT_in_active sample
    aml1218_write(0x00C9, 0x06);// open DCIN_OK and USB_OK IRQ

    aml1218_write16(0x0082, 0x0100);            // open ldo6 

    aml1218_set_bits(0x005f, 0x30, 0x7c);       // set lcd backlight level to 60uA
    /*
     * open charger
     */
...
int aml1218_init_para(struct battery_parameter *battery)
{
    if (battery) {
      ...
      //aml1218_set_full_charge_voltage(battery->pmu_init_chgvol);
        ...
        aml1218_set_trickle_time       (battery->pmu_init_chg_pretime);
        aml1218_set_rapid_time         (battery->pmu_init_chg_csttime);
...
static struct aml_pmu_driver g_aml_pmu_driver = {
    .pmu_init                    = aml1218_init, 
    .pmu_get_battery_capacity    = aml1218_get_charging_percent,
    .pmu_get_extern_power_status = aml1218_charger_online,
...
#ifdef CONFIG_USB_DWC_OTG_HCD
    .pmu_usb_bc_process          = aml1218_usb_bc_process,
#endif
    .pmu_power_off               = aml1218_power_off,
    .pmu_init_para               = aml1218_init_para,
#ifdef CONFIG_UBOOT_BATTERY_PARAMETER_TEST
    .pmu_do_battery_calibrate    = aml1218_battery_calibrate,
#endif
...
int aml1218_calculate_rdc(void)
{
    char    buf[100];
    int     i;
    int32_t i_lo, i_hi;
    ...
    v_hi = aml1218_get_battery_voltage();
    if (v_hi > 4000) {                          // to avoid PMU don't charge when battery voltage is high
        return 0;    
    }
    aml1218_set_charging_current(300 * 1000);          // charge current to 500mA 
    udelay(1000 * 1000);
    i_lo = 0;
    v_lo = 0;
    for (i = 0; i < 8; i++) {
        v_lo += aml1218_get_battery_voltage();
        i_lo += aml1218_get_battery_current();
        udelay(8000);
    }
    aml1218_set_charging_current(1050 * 1000);         // charge current to 1.05A 
    udelay(1000 * 1000);
    i_hi = 0;
    v_hi = 0;
    for (i = 0; i < 8; i++) {
        v_hi += aml1218_get_battery_voltage();
        i_hi += aml1218_get_battery_current();
        udelay(8000);
    }
    i_lo /= 8;
    v_lo /= 8;
...
int aml1218_rdc_init(void)
{
    ...
    aml1218_battery_test = 1;
    aml1218_set_bits(0x009A, 0x80, 0x80);                               // clear cloumber counter
    for (i = 0; i < 8; i++) {
        rdc_tmp = aml1218_calculate_rdc();
        if (rdc_tmp) {
            rdc_total += rdc_tmp;
            success_cnt++;
        }
...
int aml1218_update_calibrate(int charge)
{
...
int aml1218_battery_calibrate(void)
{
    ...
    aml1218_update_calibrate(1);
    prev_coulomb = coulomb;
    prev_ocv = ocv;
    prev_ibat = ibat;
    for (i = 0; i < 4; i++) {
        aml1218_update_calibrate(0);
        ocv_avg += ocv;
    }
    ...
    while (1) {
        ...
        rdc_tmp = aml1218_update_calibrate(1);
        if (rdc_tmp) {
            rdc_total += rdc_tmp; 
            rdc_cnt++;
        }
